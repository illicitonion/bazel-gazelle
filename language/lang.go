...
type Language interface {
	...
	// TODO: Document ConfigDependentLoader here. Ideally I'd like to deprecate
	// the non-Config version.
...
// ConfigDependentLoader is a Language which may generate some loads using
// information present on a File's Config object.
// Its generated output is expected to be a superset of the Language's output
// from Kinds and Loads such that this should be used instead, if a Config is available.
type ConfigDependentLoader interface {
	// ConfigDependentKinds is the same as Language.Kinds, but may use a File's
	// Config when generating its output.
	// The second argument is the relative path to the package being configured,
	// or nil if generating for repo-wide files.
	ConfigDependentKinds(*config.Config, *string) map[string]rule.KindInfo

	// ConfigDependentLoads is the same as Language.Loads, but may use a File's
	// Config when generating its output.
	// The second argument is the relative path to the package being configured,
	// or nil if generating for repo-wide files.
	ConfigDependentLoads(*config.Config, *string) []rule.LoadInfo
}

func KindsAndLoads(l Language, c *config.Config, rel *string) (map[string]rule.KindInfo, []rule.LoadInfo) {
	if cdl, ok := l.(ConfigDependentLoader); ok {
		return cdl.ConfigDependentKinds(c, rel), cdl.ConfigDependentLoads(c, rel)
	} else {
		return l.Kinds(), l.Loads()
	}
}
